...
vec3 get() {
	...
	static const char* fib_vss_body = R"GLSL(
const float pi = 3.1415926535897;
const float phi = 2.399963229728653;
...
vec3 get() {
	...
	return vec3(
		cos(longitude) * cos(latitude) * radius,
		sin(longitude) * cos(latitude) * radius,
		sin(latitude) * radius
	);
}
...
vec3 get() {
	...
	struct pointstorm : public drawable {
		...
		static std::unique_ptr<drawable> the_points_manifold() {
			return std::make_unique<kali::pointstorm>();
		}

		static std::unique_ptr<drawable> torus_demo() {
		...
		static std::unique_ptr<drawable> fib_sphere_demo() {
			return std::make_unique<kali::pointstorm>(kali::fib_vss_body, std::vector<kali::uniform>{
				uniform::make<glm::float32>("radius", 1),
				uniform::make<glm::vec4>("color", glm::vec4{ 0.0, 1.0, 0.0, 1.0 }),
			});
		}
